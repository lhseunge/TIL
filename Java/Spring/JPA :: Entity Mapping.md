...
| 분류 | 매핑 어노테이션 | 설명 |
| --- | --- | --- |
| 필드와 컬럼매핑 | @Column | 컬럼을 매핑한다. |
|  | @Enumerated | Java enum을 매핑한다. |
...
- @Column
    - nullable의 기본값은 true (null 허용)
    - Java의 데이터 타입 중 원시 타입을 사용할 때는 false로 지정하는 것이 안전하다.
- @Enumerated
    - 해당 어노테이션의 EnumType.ORDINAL, EnumType.STRING 중 ORDINARY이 적용된다.
    - ORDINAL은 enum의 순서가 DB에 저장된다.
    - 만약 Java enum의 순서가 바뀌게 되면 DB의 데이터는 기존과 같은 순서를 저장하고 있기 때문에 데이터 문제가 발생한다.
- @Access
    - 해당 어노테이션 값으로 AccessType.FIELD와 AccessType.PROPERTY 두 가지를 설정할 수 있다.
    - AccessType.FIELD는 필드에 매핑 어노테이션을 설정하는 것과 같고,
    - AccessType.PROPERTY는 getter 메소드에 매핑 어노테이션을 설정하는 것과 같다.
    - 두 가지 설정 방법을 혼용해서 사용할 수 있다
        - 필드에는 매핑 어노테이션을 설정하고,
        - 동시에 getter 메소드에 PROPERTY어노테이션을 사용하면 된다.
        - getter메소드의 메소드명에 해당하는 컬럼에 값이 주입된다.
